...
def load_image_to_batch(image):
    ...
    if landmark is not None:
        ...
        batch["landmark"] = torch.from_numpy(landmark).to('cpu')
    if seg_im is not None:
        #batch["mask"] = numpy_image_to_torch(seg_im)[None, ...].cuda()
        batch["mask"] = numpy_image_to_torch(seg_im)[None, ...].to('cpu')
    return batch
...
class TargetJawCriterion(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.reference_pose = torch.tensor(reference_pose).to('cpu')
