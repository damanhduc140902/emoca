...
def process_images(input_folder):
    # Các thiết lập cần thiết cho model EMoCA
    ...
    # Tạo dataset
    dataset = TestData(input_folder, face_detector="fan", max_detection=1)
    ...
    for i in range(len(dataset)):
        ...
        for j in range(current_bs):
            # Lưu trữ mã chi tiết từ mỗi ảnh
            all_details.append(vals["detailcode"][j].detach().cpu().numpy())
            all_exps.append(vals["expcode"][j].detach().cpu().numpy())
            all_shapes.append(vals["shapecode"][j].detach().cpu().numpy())

    return all_details, all_exps, all_shapes
