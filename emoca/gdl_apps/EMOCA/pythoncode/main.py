...
class EmotionDetectorScreen(MDScreen):
    ...
    #     # The detected emotion is stored in the label, so we retrieve it
    #     emotion = self.ids.label.text.replace('Predicted Emotion: ', '')

    #     # Create a menu based on the emotion
    #     if not self.emotion_menu:
    #         menu_items = [
    #             {
    #                 "text": f"{emotion} response {i}",
    #                 "viewclass": "OneLineListItem",
    #                 "height": dp(56),
    #                 "on_release": lambda x=f"{emotion} response {i}": self.menu_item_release(x),
    #             } for i in range(10)
    #         ]
    #         self.emotion_menu = MDDropdownMenu(
    #             caller=self.ids.bubble,
    #             items=menu_items,
    #             position="bottom",
    #             width_mult=4,
    #         )
    #     else:
    #         # Update the menu items based on the new emotion
    #         self.emotion_menu.items = [
    #             {
    #                 "text": f"{emotion} response {i}",
    #                 "viewclass": "OneLineListItem",
    #                 "height": dp(56),
...
class EmotionDetectorApp(MDApp):
    ...
    def build(self):
        ...
        Clock.schedule_once(self.show_main_screen, 10)
    ...
    def update_emoji_display(self, emotion, screen):
        ...
        screen.ids.emoji_display.text = emoji_mapping.get(emotion, "")
